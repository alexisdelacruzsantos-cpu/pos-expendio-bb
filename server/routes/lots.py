...
def add_stock():
    try:
        ...
        product = db.fetch_one('SELECT name, price, cost FROM products WHERE id = ?', (product_id,))
        if not product:
            return jsonify({'error': 'Producto no encontrado'}), 404

        if target_lot_id:
            lot = db.fetch_one(
                'SELECT * FROM lots WHERE id = ? AND product_id = ?',
                (target_lot_id, product_id)
            )
            if not lot:
                return jsonify({'error': 'El lote seleccionado no pertenece al producto'}), 400

            if float(quantity) < 0:
                return jsonify({'error': 'La cantidad a sumar debe ser positiva'}), 400

            new_qty = max(0, (lot['current_quantity'] or 0) + float(quantity))
            db.execute('UPDATE lots SET current_quantity = ? WHERE id = ?', (new_qty, lot['id']))
            log_movement(db, product_id=product_id, lot_id=lot['id'], movement_type='entry',
                         quantity=quantity, notes=f'Suma a lote existente #{lot["id"]} {(lot["batch_number"] or "")}'.strip())
            ...
            cursor = db.execute('''
                INSERT INTO lots (product_id, batch_number, expiry_date, initial_quantity, current_quantity)
                VALUES (?, ?, ?, ?, ?)
            ''', (product_id, batch_number, expiry_date, quantity, quantity))

            lot_id = cursor.lastrowid

            log_movement(db, product_id=product_id, lot_id=lot_id, movement_type='entry',
                         quantity=quantity, notes=f'Entrada de stock #{lot_id}')
        ...
        if new_cost is not None and str(new_cost) != '':
            try:
                db.execute('UPDATE products SET cost = ? WHERE id = ?', (float(new_cost), product_id))
                cost_updated = True
            except (TypeError, ValueError):
                pass
        if new_price is not None and str(new_price) != '':
            try:
                db.execute('UPDATE products SET price = ? WHERE id = ?', (float(new_price), product_id))
                price_updated = True
            except (TypeError, ValueError):
                pass

        if price_updated and float(new_price) != old_price:
            log_movement(db, product_id=product_id, movement_type='price_changed',
                         notes=f'Precio: ${old_price} → ${float(new_price)}')
        if cost_updated and float(new_cost) != old_cost:
            log_movement(db, product_id=product_id, movement_type='product_updated',
                         notes=f'Costo: ${old_cost} → ${float(new_cost)}')

        return jsonify({
            'message': 'Stock agregado exitosamente' if action == 'new_lot' else f'Stock sumado al lote {batch_number or "#"+str(lot_id)}',
            'lot_id': lot_id,
